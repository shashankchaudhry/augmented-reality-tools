    ...
    , markerSize(100,100)
{
    ...
    markerOptions.push_back(621);
    markerOptions.push_back(622);
    markerOptions.push_back(623);
    markerOptions.push_back(624);
    markerOptions.push_back(625);
    markerOptions.push_back(626);
}

void MarkerDetector::setFilePath(string filePath){
	path = filePath;
}
...
bool MarkerDetector::findMarkers(cv::Mat frame, std::vector<Marker>& detectedMarkers)
{
	...
	//std::cout<<"Prepared Image!"<<endl;

	// Make it binary
	performThreshold(m_grayscaleImage, m_thresholdImg);
	//std::cout<<"Performed Threshold!"<<endl;
...
void MarkerDetector::performThreshold(const cv::Mat& grayscale, cv::Mat& thresholdImg) const
{
    ...
    cv::showAndSave(path+"Threshold_image", thresholdImg);
#endif
}
...
#ifdef SHOW_DEBUG_IMAGES
    {
        /*ContoursVector tempContours;
        for (size_t j=0;j<contours.size();j++){
        	if(j==56){
				tempContours.clear();
				cv::Mat contoursImage(thresholdImg.size(), CV_8UC1);
				contoursImage = cv::Scalar(0);
				tempContours.push_back(contours[j]);
				cv::drawContours(contoursImage, tempContours, -1, cv::Scalar(255), 2, CV_AA);
				stringstream ss;
				ss << j;
				cv::showAndSave(path+"Contours"+ss.str(), contoursImage);
        	}
        }*/
    }
#endif
...
)
{
	...
	// We are interested only in polygons that contains only four points
        if (approxCurve.size() != 4){
        	/*if(i==56)
        		cout<<"i: "<<i<<" curve size: "<<approxCurve.size();*/
            continue;
        }
...
void MarkerDetector::recognizeMarkers(const cv::Mat& grayscale, std::vector<Marker>& detectedMarkers)
{
    ...
    for (size_t i=0;i<detectedMarkers.size();i++)
    {
...
#ifdef SHOW_DEBUG_IMAGES
        {
            ...
            cv::showAndSave(path+"Source_marker" + ToString(i), markerSubImage);
            cv::showAndSave(path+"Marker " + ToString(i) + " after_warp", canonicalMarkerImage);
        }
#endif

        int nRotations;
        int id = Marker::getMarkerId(canonicalMarkerImage, nRotations);
        if (id !=- 1 && std::find(markerOptions.begin(), markerOptions.end(), id) != markerOptions.end())
        {
            marker.id = id;
...
#if SHOW_DEBUG_IMAGES == 1
    {
        ...
        cv::showAndSave(path+"Markers_refined_edges", grayscale * 0.5 + markerCornersMat);
    }
#endif

    detectedMarkers = goodMarkers;
    listOfMarkers = goodMarkers;
}
...
std::vector<Marker> MarkerDetector::returnDetectedMarkers()
{
